...
export function ChatListScreen({ role = 'customer' }: ChatListScreenProps) {
  const navigate = useNavigate();
  const location = useLocation();

  const userRole = role || (location.pathname.includes('technician') ? 'technician' : 'customer');
  const basePath = userRole === 'customer' ? '/customer' : '/technician';
  ...
  const fetchContacts = useCallback(async () => {
    try {
      ...
      console.error('[ChatList] fetch error:', status, detail, err?.message);
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 lg:rounded-2xl lg:m-6 lg:shadow-xl shadow-lg flex items-center justify-between">
        <div>
          ...
          <p className="text-white/90">
            {userRole === 'customer' ? 'Chat with technicians' : 'Chat with customers'}
          </p>
        </div>
        <button
          onClick={() =>
            navigate(`${basePath}/chat/support`, {
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
            ...
            <p className="text-sm text-gray-500 max-w-xs">
              {userRole === 'customer'
                ? 'Once your booking is accepted, you can chat with your technician here.'
          ...
          contacts.map((contact, index) => (
            <motion.button
              ...
              onClick={() =>
                navigate(`${basePath}/chat/${encodeURIComponent(contact.email)}`, {
